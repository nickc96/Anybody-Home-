:- dynamic i_am_at/1, at/2, holding/1, atStatic/2, bill/1, billstatus/1.
...
describe(center) :-
	...
	(holding(glasses) ->
		...
		retract(holding(glasses)), assert(path(center, n, secret)), assert(path(secret, s, center)), nl, nl; true),
...
describe(kitchen) :-
	...
	(holding(key) ->
		...
		assert(path(kitchen, n, garden)), assert(path(garden, s, kitchen)), retract(holding(key)); true),
